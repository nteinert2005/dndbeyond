        ...
        ? (
            <TokenWrapper>
                <div className="modal-body p-6 flex flex-col">
                    <h1> Buy SkyKeys </h1> 
                    <span className="text-gray-400"> How many SkyKeys do you want to purchase? </span> 

                    <ul className="flex mt-4 flex-row flex-wrap"> 
                        { tokenAmount.map((item) => (
                        <li className="p-5 rounded w-full md:w-5/12 m-2 text-lg text-center hover:text-black">
                    ...
                    <XIcon className="close-btn h-8 w-8 float-right" onClick={() => closeModal()} />
                </div>
            </TokenWrapper>
        )
        : null 
